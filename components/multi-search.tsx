/**
 * This file `multi-search.tsx` defines a React component for displaying multiple search results and related images
 * in a responsive and interactive UI. The core features and structure are as follows:
 *
 * 1. **Type Definitions**
 *    - Provides TypeScript types for search results (`SearchResult`), images (`SearchImage`), multi-query responses (`MultiSearchResponse`),
 *      query parameters (`MultiSearchArgs`), and query progress (`QueryCompletion`), ensuring clear data structures.
 *
 * 2. **Loading State Component (`SearchLoadingState`)**
 *    - Displays a skeleton UI while data is loading, including query badges, result counts, and image placeholders to enhance user experience.
 *
 * 3. **Result Card (`ResultCard`)**
 *    - Renders a single search result, showing the title, content summary, source website favicon, publication date, and supports external links.
 *
 * 4. **Image Grid (`ImageGrid`)**
 *    - Displays all related images with interactive preview functionality. On desktop, uses a Dialog modal; on mobile, uses a Drawer.
 *      Supports left/right navigation and image description display.
 *
 * 5. **Main Component (`MultiSearch`)**
 *    - Accepts search results and parameters, aggregates and displays all queries' results and images.
 *    - Supports both loading and loaded states, automatically calculates total result and image counts.
 *    - Uses an `Accordion` panel to organize content for better information hierarchy.
 *
 * 6. **Animation and Styling**
 *    - Utilizes `framer-motion` for animated transitions, providing a smooth UI experience.
 *    - Employs custom UI components (such as Badge, Button, Accordion) and Tailwind CSS class names for consistent and maintainable styling.
 *
 * **Summary**:
 * This file implements a multi-query search result frontend component, integrating result cards, image previews, and loading skeletons.
 * It is suitable for scenarios requiring the display of multiple search results and related images, with adaptations for both mobile and desktop environments.
 */
